...
newprog(Prog *p, Modlink *m)
{
	...
	/*for testing limit number pid can be created*/
		//if(n->pid > 10){
...
delprog(Prog *p, char *msg)
{
	...
	/*add deleted pid to link list*/
	//print("Deleted PID: %d \n",p->pid);
	node_list * newnode = NULL;
	newnode = (node_list *)malloc(sizeof(node_list));
	newnode->pid = p->pid;
	newnode->next = NULL;	
	...
	//add terminated pid to the end of the linked list
